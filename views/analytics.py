...
def restaurant_filters(df, tab_title):
    ...
    with col1:
        ...
        selected_type = st.selectbox('Sélectionnez le type de restaurant', types, key=f"restaurant_type_{tab_title}")

    with col2:
        # Filtrer par prix
        prices = ['Tous'] + df['restaurant_price'].unique().tolist()
        selected_price = st.selectbox('Sélectionnez le prix du restaurant', prices, key=f"restaurant_price_{tab_title}")
    ...
    selected_names = st.multiselect('Sélectionnez les noms des restaurants', names, key=f"restaurant_names_{tab_title}")
...
def get_filtered_restaurant(df,selected_names, names):
    if len(selected_names) <= 0 and 'Tous' not in selected_names:
        ...
        if 'Tous' not in selected_names:
            ...
            names = [item for item in names if "Tous" not in item] # On supprime "Tous" des noms restants
            filtered_df = filtered_df[filtered_df['restaurant_name'].isin(names)]
        if len(filtered_df) > 10:
            st.warning('Vous avez sélectionné plus de dix restaurants, cela peut prendre du temps.')
...
def analytics_page(df):
    ...
    home, sentiment_analysis, word_freq_analysis, simil_anaysis = st.tabs(["Faire une analyse", "Analyse de sentiments", "Wordcloud", "Analyse de similarité"])
    with home:
...
################################################################
    ...
    with sentiment_analysis:
        ...
        st.title(TAB_TITLE)
        st.write("ℹ️ Vise à caractériser chaque restaurant à partir des sentiments véhiculés dans les avis des clients.")
        ...
        if st.button("Démarrer l'analyse", key=f"start_analysis_{TAB_TITLE}"):
            with st.spinner("Acquisition et pré-traitement des données sélectionnées... ⏳"):
            ...
            with st.spinner("Analyse des sentiments en cours... ⏳"):
                emotions_par_resto = generate_sentiments_analysis(filtered_df)
...
################################################################
    ...
    with word_freq_analysis:
        ...
        st.title(TAB_TITLE)
        st.write("ℹ️ Vise à représenter les mots les plus fréquents dans les avis des restaurants sélectionnés.")
...
################################################################
    ...
    with simil_anaysis:
        ...
        st.title(TAB_TITLE)
        st.markdown('ℹ️ Vise à représenter une "similarité" entre les restaurants à partir de la sémantique des avis des clients.  \n'+
        ...
        if st.button("Démarrer l'analyse", key=f"start_analysis_{TAB_TITLE}"):
            ...
            with st.spinner("Analyse des similarités en cours... ⏳"):
                # start = time()
                restaurant_coords, restaurant_names = generate_word2vec(filtered_df, three_dim)
                # end = time()
